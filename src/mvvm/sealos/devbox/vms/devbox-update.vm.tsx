...
import { devboxUpdateSchema } from "../models/devbox-update.model";

interface DevboxUpdateProps {
	initialData?: {
		name: string;
		resource?: {
			cpu: number;
...
export function DevboxUpdate({ initialData }: DevboxUpdateProps) {
	const form = useForm({
		defaultValues: {
		...
		validators: {
			onSubmit: ({ value }) => devboxUpdateSchema.parse(value),
		},
		onSubmit: async () => {
	...
	return (
		<Card className="w-full sm:max-w-2xl">
			<CardHeader>
				<CardTitle>Update Devbox</CardTitle>
				<CardDescription>
			...
			<CardFooter>
				<Field orientation="horizontal">
					...
					<Button type="submit" form="devbox-update">
						Update Devbox
					</Button>
				</Field>
			</CardFooter>
